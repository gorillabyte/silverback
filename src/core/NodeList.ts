...
module silverback.core {
	...
	export class NodeList {
		...
		public add(node:silverback.core.Node) {
		    if (!this.head) {
        ...
        public remove(node: silverback.core.Node) {
		    if (this.head === node) {
        ...
        public removeAll() {
		    while (this.head ) {
		        var node: silverback.core.Node = this.head;
				this.head = node.next;
				node.previous = null;
				node.next = null;
				this.nodeRemoved.dispatch( node );
		 ...
		 */
		public swap(node1:silverback.core.Node, node2:silverback.core.Node) {
		...
		public insertionSort(sortFunction) {
		    ...
		    var remains: silverback.core.Node = this.head.next;
			for (var node: silverback.core.Node = remains; node; node = remains ) {
				remains = node.next;
				for (var other: silverback.core.Node = node.previous; other; other = other.previous ) {
		...
		public mergeSort( sortFunction  ) {
			...
			var start: silverback.core.Node = this.head;
			var end: silverback.core.Node;
			while( start ) {
				end = start;
				while( end.next && sortFunction( end, end.next ) <= 0 ) {
					end = end.next;
				}
				var next: silverback.core.Node = end.next;
		...
		private _merge(head1: silverback.core.Node, head2: silverback.core.Node, sortFunction): silverback.core.Node {
		    var node: silverback.core.Node;
			var head: silverback.core.Node;
